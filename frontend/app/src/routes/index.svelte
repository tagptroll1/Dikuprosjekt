<script>
  ...
  import { getUserData, postNewUser } from "api.js";
  ...
  onMount(() => {
    ...
    function loop(t) {
      ...
      for (let p = 0; p < imageData.data.length; p += 4) {
        ...
        imageData.data[p + 0] = r * 0.9;
        imageData.data[p + 1] = g * 0.4;
        imageData.data[p + 2] = b * 0.7;
  ...
  const pattern = `[a-zA-ZæøåÆØÅ]{3}\\d{3}`; //RegEx Pattern: 3 letters and 3 numbers.
  let input;
  let value = "";
  let valid = true;
  let placeholder = "ex; rob404";
  const alternatives = ["joe432", "tom233", "aar444", "jef007"];
...
<div class="outer">
  ...
  <button disabled={!valid || null} class="start" on:click={start}>
    Start som
    <b>{value ? value : 'anonymous'}</b>
  </button>
</div>
