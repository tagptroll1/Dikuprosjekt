<script>
  ...
  $: selected =
    $question.answer && $question.answer.selected_answer && handleClick();
  ...
  //uncomment $showFeedback to display feedback after each click
  function handleClick() {
    ...
    //Uncomment the next line to show immediate feedback upon selecting a question.
    //$showFeedback = true
    showCorrect = true;
  }
</script>
...
{:then}
  ...
  {#if $showFeedback}
    <span class:correct={correct_ans}>{correct_ans ? '✔' : '✖'}</span>
    {feedback}
  {/if}
{/await}
